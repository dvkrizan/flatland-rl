...
class DDDQNPolicy(Policy):
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        #print('self.t_step is', self.t_step)
        if self.t_step == 0:
            # If enough samples are available in memory, get random subset and learn
            if len(self.memory) > self.buffer_min_size and len(self.memory) > self.batch_size:
                #print('met learning conditions')
                self._learn()
    ...
    def _learn(self):
        ...
        #print('get expected Q values')

        if self.double_dqn:
            print('is a double dqn')
            # Double DQN
            print('next_States:', self.qnetwork_local(next_states).max())
            #q_best_action = self.qnetwork_local(next_states).max(1)[1]
            q_best_action = self.qnetwork_local(next_states).max()
            print('q_best_action is', q_best_action)
            q_targets_next = self.qnetwork_target(next_states)#.gather(1, q_best_action.unsqueeze(-1))
            print('q_targets_next is', q_targets_next)
        else:
            print('is a simple DQN')
            # DQN
